...
function validateApiKey(request, env) {
  const requiredKey = env.WHOIS_API_KEY || "123456";
  const apiKey = request.headers.get('X-API-KEY');
  if (!apiKey) return createErrorResponse("未发现 API Key", 401);
  if (apiKey !== requiredKey) return createErrorResponse("无效的 API Key", 403);
  return null;
}
